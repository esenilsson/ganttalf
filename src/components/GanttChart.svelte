...
<div class="overflow-x-auto">
  ...
  >
    ...
    <!-- header labels -->
    <text x={D.activityColX} y={D.headerTop + 26} font-size="19" font-weight="700" fill="#111">Activity</text>
    {#each L.ts.months as m}
      {#if m.showLabel}
        <text x={D.chartX + m.labelX} y={D.headerTop + 26} font-size="19" font-weight="700" fill="#111">{m.label}</text>
      {/if}
    {/each}

    <!-- chart frame -->
    <rect x={D.chartX} y={D.bodyTop} width={D.chartW} height={D.bodyBottom - D.bodyTop} fill="none" stroke="#111" stroke-width="1" />

    <!-- dotted gridlines: weeks in month mode, days in week mode -->
    {#each L.dottedX as gx}
      <line x1={D.chartX + gx} y1={D.bodyTop} x2={D.chartX + gx} y2={D.bodyBottom} stroke="#999" stroke-width="0.7" stroke-dasharray="1.5 3" />
    {/each}

    <!-- week mode: solid week boundaries + W## labels -->
    {#each L.solidWeeks as w}
      <line x1={D.chartX + w.x} y1={D.headerTop + 32} x2={D.chartX + w.x} y2={D.bodyBottom} stroke="#777" stroke-width="0.8" />
    {/each}
    {#each L.weekLabels as w}
      <text x={D.chartX + w.x + 5} y={D.headerTop + 48} font-size="12" fill="#555">{w.label}</text>
    {/each}

    <!-- month boundaries (solid) -->
    {#each L.ts.months as m}
      {#if m.boundaryX != null}
        <line x1={D.chartX + m.boundaryX} y1={D.headerTop + 10} x2={D.chartX + m.boundaryX} y2={D.bodyBottom} stroke="#111" stroke-width="1" />
      {/if}
    {/each}
    ...
    {#each L.groups as g, gi}
      {#if gi > 0}
        <line x1={D.groupColX} y1={g.yTop} x2={D.totalWidth - 16} y2={g.yTop} stroke="#111" stroke-width="1" />
      {/if}
      {#if L.hasGroups}
        <text x={D.groupColX + D.groupColW - 16} y={g.labelY} text-anchor="end" dominant-baseline="middle" font-size="15" font-weight="700" fill="#111">{g.name}</text>
      {/if}
    {/each}

    <!-- rows -->
    {#each L.items as it (it.row.id)}
      <text x={D.activityColX} y={it.labelY} dominant-baseline="middle" font-size="14" fill="#111">{it.row.activity}</text>

      <!-- tentative prefix / suffix (dashed outline) -->
      {#each [it.preTentative, it.postTentative] as seg, si}
        {#if seg && seg.x1 - seg.x0 > 0}
          <g>
            <rect x={D.chartX + seg.x0} y={it.barY} width={seg.x1 - seg.x0} height={it.barH} fill="white" fill-opacity="0.01" stroke="#555" stroke-width="1.3" stroke-dasharray="5 4" />
            <rect
      ...
      {#if it.solid && it.solid.x1 - it.solid.x0 > 0}
        <rect x={D.chartX + it.solid.x0} y={it.barY} width={it.solid.x1 - it.solid.x0} height={it.barH} fill="#111" />
        <rect
      ...
      {#if it.milestoneX != null}
        ...
        {#if it.milestoneLabelLeft}
          <text x={D.chartX + it.milestoneX - 11} y={it.cy} text-anchor="end" dominant-baseline="middle" font-size="11" fill="#555">{it.milestoneLabel}</text>
        {:else}
          <text x={D.chartX + it.milestoneX + 11} y={it.cy} dominant-baseline="middle" font-size="11" fill="#555">{it.milestoneLabel}</text>
        {/if}
      {/if}

      <!-- responsible -->
      {#if it.row.responsible && it.rightEdgeX != null}
        <text x={D.chartX + it.rightEdgeX + (it.milestoneX != null ? 78 : 8)} y={it.cy} dominant-baseline="middle" font-size="11" fill="#888">{it.row.responsible}</text>
      {/if}
    {/each}
    ...
    {#each L.arrows as a}
      <polyline
        ...
        fill="none" stroke="#666" stroke-width="1.3" stroke-linejoin="round"
      />
      <polygon
    ...
    <!-- today marker -->
    {#if L.todayX != null}
      <line x1={D.chartX + L.todayX} y1={D.headerTop + 10} x2={D.chartX + L.todayX} y2={D.bodyBottom + 6} stroke="#111" stroke-width="1.6" stroke-dasharray="7 5" />
      <polygon points="{D.chartX + L.todayX - 8},{D.bodyBottom + 24} {D.chartX + L.todayX + 8},{D.bodyBottom + 24} {D.chartX + L.todayX},{D.bodyBottom + 10}" fill="#111" />
      <text x={D.chartX + L.todayX} y={D.bodyBottom + 42} text-anchor="middle" font-size="14" fill="#111">{L.todayLabel}</text>
    {/if}
  </svg>
</div>
